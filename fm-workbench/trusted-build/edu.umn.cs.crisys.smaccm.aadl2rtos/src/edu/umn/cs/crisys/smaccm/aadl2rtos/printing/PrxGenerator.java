...
public class PrxGenerator {
    ...
    private void writeUserEntrypointModules(org.w3c.dom.Element parent) {
        ...
        for (ThreadWrapper w : model.getTaskThreads()) {
            fileSet.addAll(Util.assertNonNull(w.getSourceTextListOpt(), "Thread" + (w.getName())
 ...
 */
    ...
    private void writeVectableModule(org.w3c.dom.Element parent) {
        ...
        for (ThreadWrapper i : ISRs) {
            ec = doc.createElement(Util.assertNonNull(i.getSmaccmSysSignalOpt(),
                    "ISR thread must have an associated signal"));
            ec.appendChild(doc.createTextNode(Util.assertNonNull(i.getComputeEntrypointOpt(),
                    "ISR thread must have a compute entrypoint")));
            e.appendChild(ec);
        }
    }
