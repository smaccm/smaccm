...
public class ThreadWrapper {
    ...
    public String getName() {
        return Util.normalizeAadlName(thread);
    }
    ...
    public List<PortImplWrapper> getInEventPorts() {
        ...
        for (PortImpl dpImp : dpList) {
            ...
            if (piw.getDirection() == DirectionType.IN
                    && (piw.getCategory() == PortCategory.EVENT || piw.getCategory() == PortCategory.EVENT_DATA)) {
                portList.add(piw);
            }
        }
        return portList;
    ...
    public List<PortImplWrapper> getInDataPorts() {
        ...
        for (PortImpl dpImp : dpList) {
            PortImplWrapper piw = new PortImplWrapper(model, dpImp);
            if (piw.getDirection() == DirectionType.IN && piw.getCategory() == PortCategory.DATA) {
                portList.add(piw);
            }
        }
        return portList;
    ...
    public static List<PortImplWrapper> filterPortDirection(List<PortImplWrapper> ports,
            DirectionType dir) {
    ...
    public static List<PortImplWrapper> filterPortCategory(List<PortImplWrapper> ports,
            PortCategory p) {
    ...
    public List<String> getSourceTextListOpt() {
        try {
            PropertyExpression value = PropertyUtils
                    .getSimplePropertyListValue(thread, SOURCE_TEXT);
    ...
    /*
     * public Integer getMaxExecutionTime() { getScaledRangeMaximum()
     * 
     * }
     */

    // public DispatchMechanism getDispatchMechanism() { return
    // dispatchMechanism; }
