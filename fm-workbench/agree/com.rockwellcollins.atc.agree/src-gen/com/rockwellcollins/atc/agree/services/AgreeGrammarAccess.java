...
public class AgreeGrammarAccess extends AbstractGrammarElementFinder {
	...
	public class TermExprElements extends AbstractParserRuleElementFinder {
		...
		//	"event" "(" id=NestedDotID ")" | {LatchedExpr} "latched" "(" id=NestedDotID ")" | {ThisExpr} "this" ("."
		//	subThis=NestedDotID)? | {FloorCast} "floor" "(" expr=Expr ")" | {RealCast} "real" "(" expr=Expr ")" | {RealLitExpr}
		//	val=REAL_LIT | {BoolLitExpr} val=BooleanLiteral | "(" Expr ")";
		@Override public ParserRule getRule() { return rule; }

		//{TimeExpr} "time" | ComplexExpr | {IntLitExpr} val=INTEGER_LIT | {PreExpr} "pre" "(" expr=Expr ")" | {EventExpr} "event"
		//"(" id=NestedDotID ")" | {LatchedExpr} "latched" "(" id=NestedDotID ")" | {ThisExpr} "this" ("." subThis=NestedDotID)?
		//| {FloorCast} "floor" "(" expr=Expr ")" | {RealCast} "real" "(" expr=Expr ")" | {RealLitExpr} val=REAL_LIT |
		//{BoolLitExpr} val=BooleanLiteral | "(" Expr ")"
		public Alternatives getAlternatives() { return cAlternatives; }
