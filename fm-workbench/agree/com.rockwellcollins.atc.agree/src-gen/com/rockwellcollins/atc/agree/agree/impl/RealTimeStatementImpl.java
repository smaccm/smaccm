...
public class RealTimeStatementImpl extends PatternStatementImpl implements RealTimeStatement {
	...
	public NotificationChain basicSetEvent(Expr newEvent, NotificationChain msgs) {
		...
		if (eNotificationRequired()) {
			ENotificationImpl notification = new ENotificationImpl(this, Notification.SET,
					AgreePackage.REAL_TIME_STATEMENT__EVENT, oldEvent, newEvent);
			if (msgs == null) {
				msgs = notification;
			} else {
		...
		if (newEvent != event) {
			...
			if (event != null) {
				msgs = ((InternalEObject) event).eInverseRemove(this,
						EOPPOSITE_FEATURE_BASE - AgreePackage.REAL_TIME_STATEMENT__EVENT, null, msgs);
			}
			if (newEvent != null) {
			...
			eNotify(new ENotificationImpl(this, Notification.SET, AgreePackage.REAL_TIME_STATEMENT__EVENT, newEvent,
					newEvent));
		}
	}
	...
	public NotificationChain basicSetJitter(Expr newJitter, NotificationChain msgs) {
		...
		if (eNotificationRequired()) {
			ENotificationImpl notification = new ENotificationImpl(this, Notification.SET,
					AgreePackage.REAL_TIME_STATEMENT__JITTER, oldJitter, newJitter);
			if (msgs == null) {
				msgs = notification;
			} else {
				msgs.add(notification);
	...
	public void setJitter(Expr newJitter) {
		if (newJitter != jitter) {
			NotificationChain msgs = null;
			if (jitter != null) {
				msgs = ((InternalEObject) jitter).eInverseRemove(this,
						EOPPOSITE_FEATURE_BASE - AgreePackage.REAL_TIME_STATEMENT__JITTER, null, msgs);
			}
			if (newJitter != null) {
				msgs = ((InternalEObject) newJitter).eInverseAdd(this,
						EOPPOSITE_FEATURE_BASE - AgreePackage.REAL_TIME_STATEMENT__JITTER, null, msgs);
			}
			...
			eNotify(new ENotificationImpl(this, Notification.SET, AgreePackage.REAL_TIME_STATEMENT__JITTER, newJitter,
					newJitter));
		}
	}
