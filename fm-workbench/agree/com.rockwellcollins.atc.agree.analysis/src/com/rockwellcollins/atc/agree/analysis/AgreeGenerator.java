...
public class AgreeGenerator {
    ...
    private static Program getAssumptionProgram(AgreeEmitterState state){
        
//        for(Entry<IdExpr, List<IdExpr>> entry : state.guarsForAssums.entrySet()){
//            
//            System.out.println(entry.getKey()+":");
//            for(IdExpr id : entry.getValue()){
//                System.out.print(id+", ");
//            }
//            System.out.println("\n");
//        
//        }
        ...
        //we blindly assume that all transitive subcomponents have contracts
        //so make a list to see which assumptions/guarantees we actually
        //generated in the program. Later when we reference the ordering
        //to prove assumptions we do not include guarantees of contracts
        //that were empty
        Set<String> usedAssumesAndGuarantees = new HashSet<>();
        for(VarDecl var : subNode.inputs){
            if(var.id.endsWith(guaranteeVarName) || var.id.endsWith(assumeVarName)){
                usedAssumesAndGuarantees.add(var.id);
            }
        }

        //items we change for the final node
        List<VarDecl> locals = new ArrayList<>(subNode.locals);
        List<Equation> equations = new ArrayList<>(subNode.equations);
        List<String> properties = new ArrayList<>(subNode.properties);
        ...
        IdExpr guarConjId = new IdExpr("__GUAR_CONJ");
        IdExpr guarHistId = new IdExpr("__GUAR_HIST");
        VarDecl guarConjVar = new VarDecl(guarConjId.id, NamedType.BOOL);
        VarDecl guarHistVar = new VarDecl(guarHistId.id, NamedType.BOOL);
        locals.add(guarConjVar);
        locals.add(guarHistVar);
        ...
        //create guarantee conjunction history
        equations.add(getHistEq(guarHistVar, guarConjId));
        
        
        Expr preHistGuar = new UnaryExpr(UnaryOp.PRE, guarHistId);
        preHistGuar = new BinaryExpr(new BoolExpr(true), BinaryOp.ARROW, preHistGuar);
        for(Entry<IdExpr, List<IdExpr>> entry : state.guarsForAssums.entrySet()){
            IdExpr assumId = entry.getKey();
            
            if(!usedAssumesAndGuarantees.contains(assumId.id)){
                //this component didn't actually have any assumptions
                continue;
            }
            
            List<IdExpr> guarIds = entry.getValue();
            String propRename = assumId.id.replace("__"+assumeVarName, "");
            propRename = propRename.replace("__", ".");
            propRename = propRename + " assumptions";
            
            IdExpr assumPropId = new IdExpr(assumId.id+"__PROP");
            VarDecl assumPropVar = new VarDecl(assumPropId.id, NamedType.BOOL);
            locals.add(assumPropVar);
            
            Expr assumPropExpr = new BoolExpr(true);
            ...
            state.renaming.addExplicitRename(assumPropId.id, propRename);
            state.assumProps.add(assumPropId.id);
        }
...
//        state.layout.addCategory(state.curInst.getName());
        Program assumeProgram = new Program(typeDefs, null, nodes);
        assumeProgram = new AstMapVisitor().visit(assumeProgram);

        return assumeProgram;
    }

    private static Program getSystemGuaranteeProgram(AgreeEmitterState state){
        Node subNode = nodeFromState(state);    	
        ...
        AgreeVarDecl assumHist = new AgreeVarDecl("__HIST_ASSUM", NamedType.BOOL);
        IdExpr assumHistId = new IdExpr(assumHist.id);
        locals.add(assumHist);

        Expr assums = new BoolExpr(true);
        for(Expr expr : assumptions){
            assums = new BinaryExpr(assums, BinaryOp.AND, expr);
        }

        Expr histExpr = new UnaryExpr(UnaryOp.PRE, assumHistId);
        histExpr = new BinaryExpr(histExpr, BinaryOp.AND, assums);
        histExpr = new BinaryExpr(assums, BinaryOp.ARROW, histExpr);

        equations.add(new Equation(assumHistId, histExpr));


        //get the guarantees as properties and add them to the renaming and node properties
        int i = 0;
        for(Equation eq : state.guarExpressions){
            String guarName = guaranteeVarName+i++;
            VarDecl guarVar = new VarDecl(guarName, NamedType.BOOL);
            IdExpr guarId = new IdExpr(guarVar.id);
            locals.add(guarVar);
            equations.add(new Equation(guarId, eq.expr));
            properties.add(guarId.id);

            state.renaming.addExplicitRename(guarName, eq.lhs.get(0).id);
            state.guarProps.add(guarName);
        }
        ...
        assumeGuaranteeProgram = new AstMapVisitor().visit(assumeGuaranteeProgram);
    ...
    private static Program getConsistProgram(AgreeEmitterState state){
        ...
        for(Equation subEq : state.subcomponentExprs){
            ...
            consistHistExpr = new BinaryExpr(consistHistExpr, BinaryOp.AND, subAssumId);
            ...
            Equation subHistEq = getHistEq(subHistVar, consistHistExpr);
        ...
        return new AstMapVisitor().visit(program);
    }
    ...
    public static AgreeProgram getLustre(ComponentInstance compInst, boolean monolithic){
        ...
        agreeProgram.guaranteeProgram = getSystemGuaranteeProgram(state);
        agreeProgram.consistProgram = getConsistProgram(state);
        agreeProgram.assumeProgram = getAssumptionProgram(state);
    ...
    private static void addSubcomponentNodeCall(final String prefix,
        ...
        //keep track of which guarantees are allowed to prove 
...
//        }
    ...
    private static void addAssumptionProvingMap(String prefix,
        ...
        if(state.ordering.size() != 0){
            ...
            ComponentImplementation compImpl = AgreeEmitterUtilities.getInstanceImplementation(state.curInst);
            subComps = compImpl.getAllSubcomponents();
        }

        //see which subcomponent names appear before this one in the list
        //the guarantees of the components listed before this one can be
        //used to prove this component's assumptions
        
        List<IdExpr> guarIds = new ArrayList<>();
        boolean found = false;
        for(Subcomponent subComp : subComps){
            if(subState.curComp != subComp){
                guarIds.add(new IdExpr(subComp.getName()+"__"+guaranteeVarName));
            }else{
                found = true;
                break;
            }
        ...
        state.guarsForAssums.put(new IdExpr(prefix+assumeVarName), guarIds);
        //also add any assumptions within the substate
        for(Entry<IdExpr, List<IdExpr>> entry : subState.guarsForAssums.entrySet()){
            IdExpr newId = new IdExpr(prefix+entry.getKey().id);
            List<IdExpr> newGuars = new ArrayList<>();
            for(IdExpr oldId : entry.getValue()){
                newGuars.add(new IdExpr(prefix+oldId.id));
            }
            newGuars.addAll(guarIds);
            state.guarsForAssums.put(newId, newGuars);
        }
        
        if(!found){
            throw new AgreeException("Could not find subcomponent among ordering");
        }
    ...
    private static void addNodeAndCondactCall(final String prefix,
        ...
        state.subGuarIds.add(new IdExpr(prefix+guaranteeVarName));
        state.subAssumIds.add(new IdExpr(prefix+assumeVarName));
    ...
    public static Node nodeFromState(AgreeEmitterState subState){
        ...
        //these should already be created by the constructor for the  AgreeEmitterState
        IdExpr guarId = new IdExpr(guaranteeVarName);
        IdExpr assumpId = new IdExpr(assumeVarName);
        ...
        for(Equation guarEq : subState.guarExpressions){
            ...
            guarExpr = new BinaryExpr(guarExpr, BinaryOp.AND, guarEq.expr);
        }    	
        //add guarantees of any subnodes
        for(IdExpr subGuarId : subState.subGuarIds){
            guarExpr = new BinaryExpr(guarExpr, BinaryOp.AND, subGuarId);
        }
        Expr assumExpr = new BoolExpr(true);
        for(Equation assumEq : subState.assumpExpressions){
            assumptions.add(assumEq.expr);
            assumExpr = new BinaryExpr(assumExpr, BinaryOp.AND, assumEq.expr);
        }
        //add assumptions of any subnodes
        for(IdExpr subAssumId : subState.subAssumIds){
            assumExpr = new BinaryExpr(assumExpr, BinaryOp.AND, subAssumId);
        }
        ...
        Expr guarEq = new BinaryExpr(guarId, BinaryOp.EQUAL, guarExpr);
        Expr assumEq = new BinaryExpr(assumpId, BinaryOp.EQUAL, assumExpr);

        finalAssert = new BinaryExpr(finalAssert, BinaryOp.AND, guarEq);
        finalAssert = new BinaryExpr(finalAssert, BinaryOp.AND, assumEq);
        equations.add(new Equation(assertId, finalAssert));
