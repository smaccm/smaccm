...
public class AgreeCalendarUtils {
	...
	static public Node getDFANode(String name, int synchrony) {
		...
		// ((0 -> pre r) >= 2 and p)
		Expr condExpr0 = new BinaryExpr(new BinaryExpr(rPreExpr, BinaryOp.GREATEREQUAL, intSyncValExpr), BinaryOp.AND,
				p);
		// ((0 -> pre r) <= -2 and q)
		Expr condExpr1 = new BinaryExpr(new BinaryExpr(rPreExpr, BinaryOp.LESSEQUAL, intNegSyncValxpr), BinaryOp.AND,
				q);
		...
		// r <= 2 and r >= -2
		Expr rIsBoundedExpr = new BinaryExpr(new BinaryExpr(r, BinaryOp.LESSEQUAL, intSyncValExpr), BinaryOp.AND,
