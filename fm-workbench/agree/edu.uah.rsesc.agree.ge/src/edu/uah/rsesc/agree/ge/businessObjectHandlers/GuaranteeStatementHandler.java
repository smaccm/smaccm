...
public class GuaranteeStatementHandler {
	private static final Graphic graphic = RectangleBuilder.create().build();

	@IsApplicable
	@CanDelete
	public boolean isApplicable(final @Named(Names.BUSINESS_OBJECT) GuaranteeStatement bo) {
		return true;
	}
	...
	public GraphicalConfiguration getGraphicalConfiguration(
		...
		return GraphicalConfigurationBuilder.create().graphic(graphic).annotation("<guarantee>")
				.style(StyleBuilder
	...
	public PaletteEntry[] getPaletteEntries(final @Named(Names.DIAGRAM_BO) Object diagramBo) {
		...
		return new PaletteEntry[] {
				PaletteEntryBuilder.create().label("Guarantee").category(AgreeCategories.AGREE).build()
		};
	}
	...
	public Operation buildCreateOperation(
		...
		return Operation.create(createOp -> {
			getClassifierOpBuilder().buildOperation(createOp, targetBoc.getBusinessObject())
				...
				final GuaranteeStatement newBo = AgreeFactory.eINSTANCE.createGuaranteeStatement();
						newBo.setStr("New Guarantee");
	...
	@GetName
	public String getName(final @Named(Names.BUSINESS_OBJECT) GuaranteeStatement bo) {
		return AgreeHandlerUtil.toDisplayName(bo.getStr());
	}

	@GetNameForEditing
	public String getNameForEditing(final @Named(Names.BUSINESS_OBJECT) GuaranteeStatement bo) {
		return bo.getStr();
	}

	@ValidateName
	public String validateName(final @Named(Names.BUSINESS_OBJECT) GuaranteeStatement bo,
	...
	@Rename
	public void setName(final @Named(Names.BUSINESS_OBJECT) GuaranteeStatement bo,
			final @Named(Names.NAME) String value) {
		bo.setStr(value);
	}
}
