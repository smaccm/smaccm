...
public class NodeDefExpressionHandler {
	private static final Graphic graphic = RectangleBuilder.create().build();

	@IsApplicable
	@CanDelete
	public boolean isApplicable(final @Named(Names.BUSINESS_OBJECT) NodeDefExpr bo) {
		return true;
	}
		...
		return Operation.create(createOp -> {
			getOperationBuilder().buildOperation(createOp, targetBoc.getBusinessObject())
					.map(AgreeHandlerUtil.toBusinessObjectToModify()).modifyPreviousResult(modifyBo -> {
						...
						final NodeDefExpr newBo = AgreeFactory.eINSTANCE.createNodeDefExpr();
	...
	@GetName
	public String getName(final @Named(Names.BUSINESS_OBJECT) NodeDefExpr bo) {
		return bo.getName();
	}

	@ValidateName
	public String validateName(final @Named(Names.BUSINESS_OBJECT) NodeDefExpr bo,
			final @Named(Names.NAME) String value) {
		return AgreeBusinessObjectHandlerUtil.validateName(bo, value);
	}

	@Rename
	public void setName(final @Named(Names.BUSINESS_OBJECT) NodeDefExpr bo,
			final @Named(Names.NAME) String value) {
		bo.setName(value);
	}
}
