...
their uses. **Note:** **Assume Statements (@sec:assume_statements) and Guarantee
Statements (@sec:guarantee_statements) exclusively live in *component type*s; while
Assert Statements (@sec:assert_statements) and Lemma Statements (@sec:lemma_statements)
...
(i.e., '+', '-', '\*', '/', '\^') as seen in Figure 14. **Note**: The
non-linear expression can contain references to only the input variable
(no other AGREE identifiers). Figure 15 demonstrates a linear
approximation bounding the output values within the upper and lower
bounds.

![Bound Non-linear Expression with Piecewise Linear Segments](../../media/image13.png){#fig:linearization_definition}
