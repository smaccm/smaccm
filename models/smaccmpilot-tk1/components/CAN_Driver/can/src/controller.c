...
#define can_controller_debug(fmt, args...) \
    do { \
           ZF_LOGD( A_FG_G "%s: " A_FG_RESET, __func__); \
           ZF_LOGD( fmt, ##args); \
           ZF_LOGD("\n"); \
    } while(0)
...
#define TIME_PER_TICK  DIV_ROUND(NS_IN_S, OSC_FREQ)
...
void set_baudrate(uint32_t baudrate)
{
    ...
    bit_time = DIV_ROUND(NS_IN_S, baudrate);
...
int txbuf_ready(int txb_idx)
{
    return mcp2515_read_reg(TXBCTRL(txb_idx) & TXBCTRL_TXREQ);
}
...
int txb_status(int txb_idx)
{
    ...
    can_controller_debug("line: %d status: 0x%02x\n", __LINE__, status);
...
int poll_rxbuf()
{
    uint8_t status = mcp2515_read_reg(CANINTF);

    if (status & CANINTF_RX1IF) {
        return 1;
    } else if (status & CANINTF_RX0IF) {
        return 0;
    } else {
        return -1;
    }
