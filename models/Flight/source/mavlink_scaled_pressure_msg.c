/* This file has been autogenerated by Ivory
 * Compiler version  0.1.0.0
 */
#include "mavlink_scaled_pressure_msg.h"

void mavlink_scaled_pressure_msg_send(const struct scaled_pressure_msg* n_var0,
                                      uint8_t* n_var1, uint8_t n_var2[80U])
{
    uint8_t n_local0[14U] = {};
    uint8_t* n_ref1 = n_local0;
    uint32_t n_deref2 = n_var0->time_boot_ms;
    
    mavlink_pack_uint32_t((uint8_t*) n_ref1, 0U, n_deref2);
    
    float n_deref3 = n_var0->press_abs;
    
    mavlink_pack_float((uint8_t*) n_ref1, 4U, n_deref3);
    
    float n_deref4 = n_var0->press_diff;
    
    mavlink_pack_float((uint8_t*) n_ref1, 8U, n_deref4);
    
    int16_t n_deref5 = n_var0->temperature;
    
    mavlink_pack_int16_t((uint8_t*) n_ref1, 12U, n_deref5);
    for (int32_t n_ix6 = 0; n_ix6 <= 13; n_ix6++) {
        ASSERTS(n_ix6 > 0 && 2147483647 - n_ix6 >= 6 || n_ix6 <= 0);
        if (n_ix6 + 6 >= 80) { } else {
            uint8_t n_deref7 = n_ref1[n_ix6];
            
            ASSERTS(n_ix6 > 0 && 2147483641 >= n_ix6 || n_ix6 <= 0);
            ASSERTS(0 <= 6 + n_ix6 && 6 + n_ix6 < 80);
            *&n_var2[(6 + n_ix6) % 80] = n_deref7;
        }
    }
    mavlinkSendWithWriter(29U, 115U, 14U, n_var1, n_var2);
    for (int32_t n_ix8 = 0; n_ix8 <= 57; n_ix8++) {
        ASSERTS(n_ix8 > 0 && 2147483647 - n_ix8 >= 22 || n_ix8 <= 0);
        ASSERTS(0 <= n_ix8 + 22 && n_ix8 + 22 < 80);
        *&n_var2[(n_ix8 + 22) % 80] = 0U;
    }
    return;
}

void mavlink_scaled_pressure_unpack(struct scaled_pressure_msg* n_var0, const
                                    uint8_t* n_var1)
{
    uint32_t n_r0 = mavlink_unpack_uint32_t(n_var1, 0U);
    
    *&n_var0->time_boot_ms = n_r0;
    
    float n_r1 = mavlink_unpack_float(n_var1, 4U);
    
    *&n_var0->press_abs = n_r1;
    
    float n_r2 = mavlink_unpack_float(n_var1, 8U);
    
    *&n_var0->press_diff = n_r2;
    
    int16_t n_r3 = mavlink_unpack_int16_t(n_var1, 12U);
    
    *&n_var0->temperature = n_r3;
}