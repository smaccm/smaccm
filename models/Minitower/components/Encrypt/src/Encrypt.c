/* This file has been autogenerated by Ivory
 * Compiler version  0.1.0.4
 */
#include "Encrypt.h"

static void component_entry_aux(void);

static void component_entry_aux_2(void);

void component_entry(const int64_t *n_var0)
{
    component_entry_aux();
    component_entry_aux_2();
    callback_periodic_encoder_ct_pop(n_var0);
    return;
}

void component_init(const int64_t *n_var0)
{
    callback_configureStaticKey(n_var0);
}

void component_entry_aux(void)
{
    uint8_t n_local0[80U] = {};
    uint8_t *n_ref1 = n_local0;
    bool n_r2 = tb_Encrypt_read_server2self(n_ref1);
    
    if (n_r2) {
        callback_input_tb_Encrypt_read_server2self_handler(n_ref1);
    }
    return;
}

void component_entry_aux_2(void)
{
    bool n_local0 = false;
    bool *n_ref1 = &n_local0;
    bool n_r2 = tb_Encrypt_read_uart2self(n_ref1);
    
    if (n_r2) {
        callback_input_tb_Encrypt_read_uart2self_handler(n_ref1);
    }
    return;
}
